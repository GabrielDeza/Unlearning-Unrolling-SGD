...
#First, let's compute the gradient before hand
M_pretrain.train()
for i,(img,label) in enumerate(unlearned_loader):
    ...
    if args.loss_func == 'hess':
        loss_unl = my_cross_entropy(out_pre,label)
    if args.loss_func =='std':
        loss_unl = std_loss(output_pre,label)
    if args.loss_func =='regular':
        loss_unl = loss_fn(output_pre,label)
    ...
    for main_idx,(inputs, targets) in enumerate(data_loader):
        ...
        if main_idx%args.eval_every == 0:
            ...
            for i, (name, params) in enumerate(M.named_parameters()):
                ...
                old_params[name] += (ep+1) * lr * grads[i]
...
import pickle
if not os.path.isdir('hessian_and_l2_loss_results'):
    os.mkdir('hessian_and_l2_loss_results')
path = f'./hessian_and_l2_loss_results/{args.model}_{args.dataset}_{args.loss_func}_{args.pretrain_batch_size}_{args.pretrain_epochs}_{args.finetune_batch_size}_{args.finetune_epochs}_{args.regularizer}_{args.l2_regularizer}_{args.eval_every}.p'
